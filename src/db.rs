...
impl Cache {
    ...
    fn clear_matches_file_if_new_hour(
        &mut self,
    ...
    pub fn is_cached(&self, x: i32, y: i32) -> Result<bool> {
        ...
        let mut stmt = self
            .conn
            .prepare("SELECT 1 FROM tiles WHERE tile_x = ? AND tile_y = ? AND created_at > ?")?;

        let exists = stmt.exists(rusqlite::params![x, y, cutoff_time])?;
        Ok(exists)
    }

    pub fn mark_cached(&self, x: i32, y: i32, response_code: i32, url: &str) -> Result<()> {
        self.conn.execute(
    ...
    pub fn mark_match(&mut self, x: i32, y: i32, response_code: i32, url: &str) -> Result<()> {
    ...
    pub fn is_match_exists(&self, url: &str) -> Result<bool> {
        let mut stmt = self.conn.prepare("SELECT 1 FROM matches WHERE url = ?")?;
    ...
    pub fn is_processed(&self, x: i32, y: i32) -> Result<bool> {
        let mut stmt = self
            .conn
            .prepare("SELECT 1 FROM processed_tiles WHERE tile_x = ? AND tile_y = ?")?;
    ...
    pub fn get_all_processed_tiles(&self) -> Result<Vec<(i32, i32)>> {
        let mut stmt = self
            .conn
            .prepare("SELECT tile_x, tile_y FROM processed_tiles")?;
    ...
    pub fn cleanup_expired_entries(&mut self) -> Result<(u64, u64)> {
        ...
        let tiles_deleted = self
            .conn
            .execute("DELETE FROM tiles WHERE created_at <= ?", [cutoff_time])?
            as u64;

        // Delete expired matches (matches don't expire, but we'll use same TTL for consistency)
        let matches_deleted = self
            .conn
            .execute("DELETE FROM matches WHERE created_at <= ?", [cutoff_time])?
            as u64;
    ...
    pub fn get_all_cached_tiles(&self) -> Result<Vec<(i32, i32)>> {
        ...
        let mut stmt = self
            .conn
            .prepare("SELECT tile_x, tile_y FROM tiles WHERE created_at > ?")?;

        let rows = stmt.query_map([cutoff_time], |row| Ok((row.get(0)?, row.get(1)?)))?;
    ...
    pub fn get_fetch_progress(&self) -> Result<(u64, u64, Option<(i32, i32)>, Option<u64>)> {
        ...
        // For row-by-row traversal, we want the tile with the highest x (row), and within that row, highest y (column)
        let mut stmt = self
            .conn
            .prepare("SELECT MAX(tile_x), MAX(tile_y) FROM tiles WHERE created_at > ?")?;

        let mut max_coords: Option<(i32, i32)> = None;
        let mut rows = stmt.query_map([cutoff_time], |row| {
            Ok((row.get::<_, Option<i32>>(0)?, row.get::<_, Option<i32>>(1)?))
        })?;

        // Get the actual max coordinates
        ...
        // Convert max coords to index: index = x * 2048 + y (row-by-row traversal)
        // Maximum valid index is 2047 * 2048 + 2047 = 4194303
        let last_index = max_coords.map(|(x, y)| {
            let x = x.min(2047).max(0) as u64;
            let y = y.min(2047).max(0) as u64;
            let calculated_index = (x * 2048) + y + 1; // +1 to start from next tile
            // Clamp to valid range [0, 4194303] (but we add 1, so clamp to [0, 4194304], then clamp to 4194303)
            calculated_index.min(4194303)
        });

        Ok((total_fetched, total_200s, max_coords, last_index))
    }

    pub fn get_tile_counts(&self) -> Result<(u64, u64)> {
        // Returns (indexed_tiles_count, processed_tiles_count)
        // Only counts non-expired entries for indexed tiles
        let cutoff_time = self.get_cutoff_time();

        // Count indexed tiles (non-expired)
        let indexed_count: u64 = self
            .conn
            .query_row(
                "SELECT COUNT(*) FROM tiles WHERE created_at > ?",
                [cutoff_time],
                |row| row.get(0),
            )?;

        // Count processed tiles (no expiration check)
        let processed_count: u64 = self
            .conn
            .query_row(
                "SELECT COUNT(*) FROM processed_tiles",
                [],
                |row| row.get(0),
            )?;

        Ok((indexed_count, processed_count))
    }
}
