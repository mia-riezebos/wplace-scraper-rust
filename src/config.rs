...
impl Config {
    pub fn from_env() -> Result<Self, Box<dyn std::error::Error + Send + Sync>> {
        ...
        // Ensure outputs directory exists for debug.txt
        ...
        let api_endpoint = env::var("WPLACE_API_ENDPOINT")
            ...
            .map_err(|_| {
                "WPLACE_API_ENDPOINT (or API_ENDPOINT) environment variable is required".to_string()
            })?;

        let tile_path = env::var("WPLACE_TILE_PATH")
            .or_else(|_| env::var("TILE_PATH"))
            .map_err(|_| {
                "WPLACE_TILE_PATH (or TILE_PATH) environment variable is required".to_string()
            })?;
        ...
        let match_threshold = env::var("MATCHER_THRESHOLD")
            ...
            .map_err(|e| {
                format!(
                    "MATCHER_THRESHOLD must be a valid number between 0 and 1: {}",
                    e
                )
            })?;

        if match_threshold < 0.0 || match_threshold > 1.0 {
            return Err("MATCHER_THRESHOLD must be between 0 and 1"
                .to_string()
                .into());
        }

        let tile_selection_mode_str = env::var("FETCHER_TILE_SELECTION_MODE")
            .or_else(|_| env::var("WPLACE_TILE_SELECTION_MODE"))
            .or_else(|_| env::var("TILE_SELECTION_MODE"))
            .unwrap_or_else(|_| "random".to_string())
            .to_lowercase();

        let tile_selection_mode = match tile_selection_mode_str.trim() {
            "consecutive" | "sequential" => TileSelectionMode::Consecutive,
            "grid" => TileSelectionMode::Grid,
            "random" | _ => TileSelectionMode::Random,
        };

        // Debug: Log the tile selection mode value for troubleshooting
        let debug_msg = format!("[DEBUG] Tile selection mode env var: '{}' -> {:?}\n", tile_selection_mode_str.trim(), tile_selection_mode);
        utils::write_debug(&debug_msg);
        ...
        if proxy_mode == ProxyMode::Rotating {
            if proxy_endpoint.is_none() {
                return Err("PROXY_ENDPOINT is required when PROXY_MODE=rotating"
                    .to_string()
                    .into());
            }
            if proxies_count.is_none() {
                return Err("PROXY_COUNT is required when PROXY_MODE=rotating"
                    .to_string()
                    .into());
            }
        }
